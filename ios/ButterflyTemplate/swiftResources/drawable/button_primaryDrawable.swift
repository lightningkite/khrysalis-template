...
static let button_primary: Drawable = Drawable { (view: UIView?) -> CALayer in 
    let layer = CALayer()
    let part1: CALayer = {
        let sublayer = button_primaryPart1.makeLayer(view)
    ...
    let part2: CALayer = {
        let sublayer = button_primaryPart2.makeLayer(view)
        layer.bounds.size = layer.bounds.size.expand(sublayer.bounds.size)
        layer.onResize.startWith(layer.bounds).addWeak(sublayer) { (sublayer, bounds) in sublayer.resize(bounds) }
        return sublayer
    }()
    let part3: CALayer = {
        let sublayer = button_primaryPart3.makeLayer(view)
        layer.bounds.size = layer.bounds.size.expand(sublayer.bounds.size)
        layer.onResize.startWith(layer.bounds).addWeak(sublayer) { (sublayer, bounds) in sublayer.resize(bounds) }
        return sublayer
    }()
...
static let button_primaryPart1: Drawable = Drawable { (view: UIView?) -> CALayer in 
    let layer = CALayer()
applyColor(view, R.color.colorPrimaryDark) { (c, s) in
...
static let button_primaryPart2: Drawable = Drawable { (view: UIView?) -> CALayer in 
    let layer = CALayer()
...
static let button_primaryPart3: Drawable = Drawable { (view: UIView?) -> CALayer in 
    let layer = CALayer()
