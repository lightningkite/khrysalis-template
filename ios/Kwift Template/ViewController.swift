...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = ResourcesColors.colorPrimaryDark
        ...
        outerView.backgroundColor = ResourcesColors.colorPrimaryDark
        
        let m = ViewController.main.generate(dependency)
        innerView = m
        outerView.flex.addItem(m).position(.absolute).left(0).top(0).right(0).bottom(0)
    ...
    override func viewDidLayoutSubviews() {
        guard let outerView = outerView, let selfView = self.view else { return }
        outerView.frame.origin.y = UIApplication.shared.statusBarFrame.height
        outerView.frame.size.width = self.view.frame.size.width
        outerView.frame.size.height = self.view.frame.size.height - outerView.frame.origin.y
        outerView.flex.layout()
    }
