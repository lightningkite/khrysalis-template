...
public enum HttpClient {
    ...
    static func call<T: Codable>(
        ...
        body: Codable? = nil,
        onResult: @escaping (Int, T?, String?) -> Void
    ) {
                ...
                if let casted = response as? HTTPURLResponse, let data = data {
                    ...
                    if casted.statusCode / 100 == 2 {
                        do {
                            ...
                            onResult(casted.statusCode, parsed, nil)
                        } catch {
                            print("HttpClient: Failed to parse due to: \(error.localizedDescription)")
                            onResult(casted.statusCode, nil, error.localizedDescription)
                        }
                    } else {
                        onResult(casted.statusCode, nil, dataString)
                    }
                } else {
    ...
    static func call(
        ...
        headers: [String: String] = [:],
        body: Codable? = nil,
        onResult: @escaping (Int, Empty?, String?) -> Void
    ) {
        print("HttpClient: Sending \(method) request to \(url) with headers \(headers)")
        let urlObj = URL(string: url)!
        ...
        let completionHandler = { (data:Data?, response:URLResponse?, error:Error?) in
            DispatchQueue.main.async {
                if let casted = response as? HTTPURLResponse, let data = data {
                    ...
                    print("HttpClient: Response \(casted.statusCode): \(dataString)")
                    if casted.statusCode / 100 == 2 {
                        onResult(casted.statusCode, (), nil)
                    } else {
                        onResult(casted.statusCode, (), dataString)
                    }
                } else {
                    print("HttpClient: ERROR!  Response is not URLResponse")
                    onResult(0, nil, "Failed")
