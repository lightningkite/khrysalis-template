...
open class AbstractCalendarView : UIView {
    ...
    var headerFont: Int = 16
    var labelFont: Int = 12
    var dayFont: Int = 16
        ...
        flex.direction(.column).alignItems(.stretch).define { flex in
            ...
            flex.addItem({ () -> UICollectionView in
                ...
                currentPage.addAndRunWeak(sub) { sub, value in
                    ...
                    sub.scrollToItem(at: IndexPath(row: Int(value), section: 0), at: .centeredHorizontally, animated: true)
                }
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1, execute: {
                    sub.scrollToItem(at: IndexPath(row: Int(self.currentPage.value), section: 0), at: .centeredHorizontally, animated: false)
                })
                sub.addOnLayoutSubviews { [weak sub] in
                    (sub?.collectionViewLayout as? ViewPagerLayout)?.measure()
                }
                sub.whenScrolled { [weak self] index in
                    suppress = true
                    self?.currentPage.value = index
                    suppress = false
                }
                return sub
            }()).grow(1)
